...
class AddHAWBDialog(QDialog):
    def __init__(self, parent=None):
        ...
        form = QFormLayout()
        form.setSpacing(10)
        self.hawb_input = QLineEdit(); self.hawb_input.setPlaceholderText("Número (Ej: HAWB123)"); self.hawb_input.setMinimumHeight(32)
        ...
        form.addRow("Número:", self.hawb_input); form.addRow("Piezas:", self.pieces_input); layout.addLayout(form)
        layout.addStretch()
...
class LabelGeneratorWidget(QWidget):
    def __init__(self, username):
        ...
        header = QHBoxLayout()
        title_lbl = QLabel("Generar Nueva Guía Master"); title_lbl.setStyleSheet("font-size: 24px; font-weight: bold; color: #222;")
        ...
        # 2. HAWB
        gb_hawb = QGroupBox("Guías Hijas (Opcional)"); gb_hawb.setObjectName("Card")
    ...
    def add_hawb_dialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            for row in self.hawb_rows:
                if row["hawb"].strip().upper() == new_hawb: QMessageBox.warning(self, "Duplicado", f"La HAWB '{new_hawb}' ya está en la lista."); return
    ...
    def save_data(self):
        ...
        if not prefix or not number or not origin or not dest or total == 0: QMessageBox.warning(self, "Atención", "Completa todos los campos obligatorios."); return
        ...
        try:
            ...
            if conn: conn.close()
            QMessageBox.critical(self, "Error de Conexión", f"Error: {e}"); return
