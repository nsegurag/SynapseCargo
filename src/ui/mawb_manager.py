...
class MAWBManager(QWidget):
    def __init__(self, username="admin"):
        ...
        self.search_input.setMinimumWidth(300); self.search_input.setMinimumHeight(40)
        ...
        self.btn_logs.setCursor(Qt.CursorShape.PointingHandCursor); self.btn_logs.setMinimumHeight(40); self.btn_logs.setStyleSheet(BTN_WHITE)
    ...
    def load_data(self):
        ...
        try:
            ...
            # 🔥 CORRECCIÓN: Quitamos 'status' para evitar error si no existe la columna
            ...
            for r_idx, row in enumerate(rows):
                ...
                mawb_item = QTableWidgetItem(row[1]) # MAWB
                mawb_item.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
                self.table.setItem(r_idx, 1, mawb_item)
                
                self.table.setItem(r_idx, 2, QTableWidgetItem(f"{row[2]} ➝ {row[3]}")) # Ruta
                self.table.setItem(r_idx, 3, QTableWidgetItem(row[4])) # Service
                self.table.setItem(r_idx, 4, QTableWidgetItem(f"{row[5]} pcs")) # Total
                
                size_val = row[6] if row[6] else "4x6"
                self.table.setItem(r_idx, 5, QTableWidgetItem(size_val)) # Tamaño
    ...
    def on_row_selected(self):
        ...
        # BOTÓN MANIFIESTO (Estilo Naranja)
        btn_manifest = QPushButton("📋 Manifiesto PDF")
        btn_manifest.setMinimumHeight(45)
        ...
        m_btns = QHBoxLayout()
        btn_edit_m = QPushButton("Editar Master"); btn_edit_m.setStyleSheet(BTN_WHITE)
        btn_edit_m.clicked.connect(lambda: self.edit_mawb_action(master_id))
        
        btn_del_m = QPushButton("Eliminar Todo"); btn_del_m.setStyleSheet(BTN_RED)
        ...
        # AGREGAR BOTÓN MANIFIESTO AL FINAL
    ...
    def add_hawb(self, master_id, mawb_num):
        ...
        if dialog.exec():
            ...
            try:
                ...
                conn.commit(); conn.close()
                self.load_data(); self.table.selectRow(current_row); self.on_row_selected() 
    ...
    def edit_hawb(self, master_id, mawb_num):
        ...
        if row == -1: QMessageBox.warning(self, "Aviso", "Selecciona una HAWB de la lista."); return
        hid, hnum, hpcs = self.current_hawbs_data[row]
    ...
    def delete_hawb(self, master_id, mawb_num):
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            ...
            cursor.execute("SELECT SUM(pieces) FROM houses WHERE master_id=%s", (master_id,))
            res = cursor.fetchone()[0]; new_total = res if res else 0
            cursor.execute("UPDATE masters SET total_pieces=%s WHERE id=%s", (new_total, master_id))
            self.regenerate_labels_db(cursor, master_id, mawb_num, new_total)
            conn.commit(); conn.close()
            self.load_data(); self.table.selectRow(current_row); self.on_row_selected() 
    ...
    def delete_mawb_action(self, master_id, mawb_num):
        confirm = QMessageBox.question(self, "Eliminar Todo", f"¿Estás seguro de eliminar la MAWB {mawb_num}?\nSe borrarán todas sus HAWBs y etiquetas.", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if confirm == QMessageBox.StandardButton.Yes:
            try:
                conn = get_db_connection(); cursor = conn.cursor()
                cursor.execute("DELETE FROM labels WHERE master_id=%s", (master_id,))
                ...
                self.load_data()
                self.clear_right_panel()
                lbl_deleted = QLabel("Guía Eliminada 🗑️"); lbl_deleted.setAlignment(Qt.AlignmentFlag.AlignCenter)
                lbl_deleted.setStyleSheet("color: #D32F2F; font-size: 18px; font-weight: bold;")
                self.right_layout.addWidget(lbl_deleted); self.right_layout.addStretch()
            except Exception as e: QMessageBox.critical(self, "Error", str(e))
